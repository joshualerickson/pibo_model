...
# Train the model
set.seed(136564)
lasso_model <- train(gr_95_bss[,2:7],
                       gr_95_bss$mean_bf,
...
lasso_model
plot(lasso_model)
varImp(lasso_model$finalModel, lasso_model$bestTune$lambda)
...
# Train the model
set.seed(136564)
lasso_model_ba <- train(gr_95_bss[,2:6],
...
lasso_model_ba
plot(lasso_model_ba)
varImp(lasso_model_ba$finalModel, lasso_model_ba$bestTune$lambda)
...
### w/d section
gr_95_bss <- gr_95 %>% select(mean_wd_2009,'fac_taudem_17all_int','us_precip_1981_2010_cpg_all', 'ave_ann_cwd', 'swe_avg_0301_cpg_all', 'clay_100_cpg_all', 'slope_percent_int_cpg_all',
                              MAP_UNIT_N) %>% 
...
# Train the model
set.seed(136564)
lasso_model_wd <- train(gr_95_bss[,2:7],
                       gr_95_bss$mean_wd_2009,
                        method = "glmnet",metric = "RMSE",
                        tuneGrid = expand.grid(alpha = 1,lambda = seq(0.001,0.1,by = 0.001)),
                        trControl = train.control
                    )
lasso_model_wd
plot(lasso_model_wd)
varImp(lasso_model_wd$finalModel, lasso_model_wd$bestTune$lambda)
...
check_normality(mod_wd)
#normal

check_heteroscedasticity(mod_wd)
#hetereoscedastic 
```
